...
$ParentDir = (Resolve-Path -LiteralPath $WorkspaceRoot).Path
$OutputPath = [System.IO.Path]::GetFullPath($OutputPath)
$OutputDirectory = Split-Path -Parent $OutputPath
if (-not (Test-Path -LiteralPath $OutputDirectory -PathType Container)) {
    throw "Dashboard output directory does not exist: $OutputDirectory"
}

$DashboardDirectoryName = Split-Path -Leaf $PSScriptRoot

function Test-SensitiveRepositoryPath {
    param([string]$Path)

    $normalized = $Path -replace '^[ MADRCU?!]{1,2}\s+', ''
    $normalized = $normalized.Trim('"')
    return $normalized -match '(?i)(^|[\\/])(?:\.env(?:\.[^\\/]*)?|secrets?|credentials?)([\\/]|$)|\.(?:pem|key|pfx|p12)$'
}

function ConvertTo-PublicFileLabel {
    param([string]$Path)

    if (Test-SensitiveRepositoryPath -Path $Path) {
...
$directories = Get-ChildItem -Path $ParentDir -Directory | Where-Object {
    $_.Name -notmatch "^\." -and
    $_.Name -ne $DashboardDirectoryName -and
    ...
    if ($hasGit) {
        ...
        if ($statusLines) {
            ...
            foreach ($line in ($statusLines | Select-Object -First 8)) {
                $uncommittedFiles += ConvertTo-PublicFileLabel -Path $line.Trim()
            }
        }
        ...
        if ($changedRaw) {
            foreach ($cr in $changedRaw) { $lastCommitFiles += ConvertTo-PublicFileLabel -Path $cr }
        }
        ...
        if ($remotes) {
            ...
            if ($RefreshRemote) {
                ...
                & git -C $path fetch --quiet --no-tags 2>$null
            ...
            if (-not $remoteAvailable) {
                $remoteStatus = "Unknown (fetch failed)"
                $healthBadge = "[!] Remote unavailable"
            } elseif ($statusSb -match "behind (\d+)") {
...
# Export one public dashboard snapshot. Do not write compatibility copies to the workspace root.
